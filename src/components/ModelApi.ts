...
export class ApiModel extends Api implements IApiModel {
	...
	async postOrder(order: IOrderedLot): Promise<IOrderResult> {
		...
		return { id: response.id, total: response.total };
	}

	async fetchItems(): Promise<IGoodsItem[]> {
		const { items } = (await this.get('/product')) as { items: IGoodsItem[] };
		if (!Array.isArray(items)) throw new Error('Неверный формат ответа');
